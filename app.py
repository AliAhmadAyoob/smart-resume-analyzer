...
with st.sidebar:
    ...
    st.divider()
    st.info("The analyzer compares your resume against the job description using NLP to provide an ATS score.")
...
if uploaded_file:
    ...
    overall_score = (
        ...
        m1.metric("Skills", f"{scores['skills_score']}%")
        m2.metric("Experience", f"{scores['experience_score']}%")
        m3.metric("Projects", f"{scores['project_score']}%")
        m4.metric("Contact", f"{scores['contact_score']}%")
